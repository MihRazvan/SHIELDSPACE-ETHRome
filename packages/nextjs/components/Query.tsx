...
const QueryEvent = () => {
  ...
  const handleQuery = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contractAddress = process.env.NEXT_PUBLIC_SECRET_ADDRESS;
      const codeHash = process.env.NEXT_PUBLIC_CODE_HASH;

      if (!contractAddress || !codeHash) {
        throw new Error("Missing environment variables for contract address or code hash.");
      }
      ...
      <div className="mt-4">
        <form onSubmit={handleQuery} className="space-y-4" style={{ width: "460px" }}>
          <div className="border-4 border-brand-orange rounded-lg p-4">
            <div>
              <label className="block text-sm font-medium leading-6 w-full">Key</label>
              <input
                type="text"
                value={key}
            ...
            <div>
              <label className="block text-sm font-medium leading-6 w-full">Viewing Key</label>
              <input
                type="text"
                value={viewingKey}
